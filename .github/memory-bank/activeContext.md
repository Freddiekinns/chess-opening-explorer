...
Merging #92 moved `main` again and three more PRs arrived, all cut from the tip
`fc1a43c9` — base SHAs checked, so the stale-branch trap did not apply. #93
(`eslint-plugin-react-refresh` 0.4.26) and #95 (`concurrently` 10.0.5) merged;
#94 was answered by deleting the dependency, in #96.

**#93 is the `ignore` entry behaving.** It pins `>=0.5.0`, so the 0.4 line still
flows as an ordinary `npm-development` bump and Lint stays green — the same
check that produced 149 errors on #76 and #89.
...
so is `packages/api`, there is no `@jest-environment` pragma anywhere, and
`packages/web` runs Vitest against its own `jsdom@23`.
`jest-environment-jsdom@30` beside `jest@29` would have been a live mismatch.
Dropped with `cross-env` in #96, because **deleting the dependency is the only
thing that stops Dependabot proposing it** — closing suppresses one version, per
#76 → #89.

**Two open decisions.** `open-pull-requests-limit` is 5 and counts open PRs per
ecosystem, so #77 and #78 hold two npm slots until #86 lands — `react-router@7`
still has not been filed. And root `engines.node` says `>=18.0.0` while
concurrently 10 declares `>=22` and CI pins 20; nothing enforces it, but the
declaration needs to become `>=20` or `>=22` with CI moved to match.
