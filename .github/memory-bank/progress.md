...
- **Study Matching V2** (2026-07-10/11, branch `feat/study-matching-v2`):
  cached fetch + offline `course:rematch`, multi-anchor scored matcher with
  family guard, schema v2 + study-level cards with badges, dual-schema audit
  script. Coverage 18.2%→35.7% all / 62.5%→91.5% top-200; contamination
  5.8%→0; dupes 1,329→0; title dupes→0. Also fixed path-resolver in git
  worktrees. Follow-up: pruned 190 dead studies, ran `course:discover` for 14
  more (coverage →36.4%/92.0%). Report: `docs/reviews/2026-07-10-study-matching-v2.md`.
