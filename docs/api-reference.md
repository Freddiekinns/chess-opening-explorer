...
### **Static Data Serving (for Detail Pages)**
```typescript
interface StaticData {
  'GET /api/openings/{fen-sanitized-id}.json': {
    purpose: 'Get complete opening data for a detail page, served statically'
    generation: 'Pre-generated by `tools/database/static-file-generator.js`'
    serving: 'Served directly by the web server (e.g., Express static) without hitting Node.js logic'
    benefits: 'Extremely fast, cacheable at CDN, zero backend load'
    response: 'A single, complete ChessOpening object with all related data'
  }
}
```
...
interface VideoAccessService {
  ...
  dataSources: [
    'data/Videos/*.json (Individual FEN-based video files)'
  ],
  
  caching: 'In-memory cache for recently accessed positions',
}
```
