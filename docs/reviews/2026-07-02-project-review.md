...
generated, so none of June's matching work has reached users** (verified in the
companion video review, `2026-07-02-video-experience-review.md`), and every
popularity statistic on the site is dated 2025-07-15.
...
3. **Shipped-but-not-delivered work and stale data have no owner** — merged
   ...
   process watches them (§1.4 and the companion video review).
...
- ⚠️ Cross-family video matches — fixed in code (PRs merged), **but verified
  ...
  All of June's matcher work is invisible to users. Ship checklist in the
  companion video review (`2026-07-02-video-experience-review.md`, §2).
...
| Horizon             | Work                                                                                                                                                                                                                                                                       |
| ------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Now (days)**      | Run the video pipeline + rematch and ship the index (video review §2 — it was never run). Refresh popularity stats. Study title dedupe + family guard. Delete `/api/openings/all`. Browser caching + `fields=lookup` on Analyse. Console.log sweep. Audio files in or out. |
...
## 6. Video experience — see the companion review

The video findings grew large enough to deserve their own document:
**`docs/reviews/2026-07-02-video-experience-review.md`**. Headlines:

- **Verified: the improved matching never shipped.** Both index copies are
  byte-identical and stamped 2026-03-15 — before the June matcher PRs merged.
  The live site still serves the old baseline (28.2% coverage, 7.9% cross-family
  contamination = 1,577 wrong-family matches). The companion doc has a 5-step
  local ship checklist (backfill → pipeline → audit → copy → commit).
- **Discovery plan (V1–V6)**: family fallback for empty galleries, "why this
  video" badges, embedded player + watched state, per-family video shelves,
  level/format classification, and chapter-level matching (deep-link `?t=` into
  survey videos — turns the fan-out problem into precise coverage).
- **Pipeline**: variation-level classification + chapters as one project;
  coverage gaps solved in the UI (V1), not by loosening thresholds; monthly
  freshness automation (S1).
