...
interface Opening {
  ...
  analysis_json?: {
    // Changed from analysis to analysis_json to match API
    description?: string;
    style_tags?: string[];
    popularity?: number;
    complexity?: string;
      ...
      try {
        ...
        if (data.success) {
          ...
          if (selectedCategory !== 'all') {
            filtered = flattenedOpenings.filter(
              (opening) => opening.eco && opening.eco.startsWith(selectedCategory)
            );
          }
          ...
          if (selectedCategory !== 'all' && deduplicated.length < 5) {
            ...
            if (fallbackData.success) {
              let fallbackFiltered = fallbackData.data.filter(
                (opening: Opening) => opening.eco && opening.eco.startsWith(selectedCategory)
              );
              ...
              const fallbackUnique = fallbackFiltered.reduce(
                (acc: Record<string, Opening>, opening: Opening) => {
                  const key = opening.fen;
                  if (!acc[key]) {
                    acc[key] = opening;
                  }
              ...
              fallbackDeduplicated.sort(
                (a, b) => (b.games_analyzed || 0) - (a.games_analyzed || 0)
              );
    ...
    const updatedCategories = [
      ...
      ...ecoCategories.map((ecoLetter) => {
        ...
        const categoryOpenings = openings.filter(
          (opening) => opening.eco && opening.eco.startsWith(ecoLetter)
        );
        const uniqueInCategory = categoryOpenings.reduce(
          (acc, opening) => {
            const key = opening.fen;
            if (!acc[key] || (opening.games_analyzed || 0) > (acc[key].games_analyzed || 0)) {
              acc[key] = opening;
  ...
  return (
    <section className={`popular-openings-section ${className}`}>
      ...
      <div
        ...
        key={`${selectedCategory}-${selectedComplexity}-${displayLimit}`}
      >
