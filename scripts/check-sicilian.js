...
const sicilianEntries = Object.entries(data).filter(
  ([fen, entry]) => entry.name === 'Sicilian Defense'
);
...
sicilianEntries.forEach(([fen, entry]) => {
  ...
  console.log(
    `- ${entry.eco}: ${hasFullAnalysis ? '✅ Complete' : '❌ Missing fields'} - Fields: ${Object.keys(analysis).join(', ')}`
  );
