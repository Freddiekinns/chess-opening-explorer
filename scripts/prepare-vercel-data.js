...
const filesToCopy = ['popularity_stats.json', 'most_popular_openings.json'];

// Additional data files already live in api/data/ (the canonical data
// location) — nothing to stage. Kept as an empty list so a future generated
// file can be added back easily.
...
// Copy additional files (with custom source paths)
additionalFiles.forEach(({ source, target, name }) => {
  const sourcePath = path.join(ROOT_DIR, source);
  const targetPath = path.join(TARGET_DATA_DIR, target);

  if (fs.existsSync(sourcePath)) {
    fs.copyFileSync(sourcePath, targetPath);
    const stats = fs.statSync(targetPath);
    console.log(`✅ Copied ${name} (${(stats.size / 1024 / 1024).toFixed(2)} MB)`);
  } else {
    console.log(`⚠️  Warning: ${name} not found at ${sourcePath}`);
  }
});
...
dirsToCopy.forEach((dirname) => {
  ...
  if (fs.existsSync(sourcePath)) {
    ...
    console.log(
      `✅ Copied ${dirname}/ directory: ${fileCount} files (${(totalSize / 1024 / 1024).toFixed(2)} MB)`
    );
  } else {
...
if (fs.existsSync(videosSourcePath)) {
  ...
  console.log(
    `📺 Videos directory: ${videoFiles.length} files (${(totalVideoSize / 1024 / 1024).toFixed(2)} MB)`
  );
  console.log(
    `⚠️  Note: Videos directory too large for Vercel deployment (${(totalVideoSize / 1024 / 1024).toFixed(0)}MB)`
  );
...
console.log(`   • Video index: Consolidated for production deployment`);
console.log(`   • Courses data: Ready`);
