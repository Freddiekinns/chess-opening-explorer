    ...
    for (const query of testQueries) {
      ...
      try {
        ...
        if (results.results && results.results.length > 0) {
          console.log(
            `✅ Found ${results.results.length} results (${results.searchType || 'unknown'} search)`
          );

          // Show top 3 results
          results.results.slice(0, 3).forEach((opening, index) => {
            const score = opening.searchScore
              ? ` (${(opening.searchScore * 100).toFixed(1)}%)`
              : '';
          ...
          if (results.queryIntent) {
            console.log(
              `   Intent: ${results.queryIntent.type} - ${JSON.stringify(results.queryIntent.style || [])}`
            );
          }
        } else {
    ...
    try {
      ...
      for (const partialQuery of suggestionQueries) {
        ...
        if (suggestions.length > 0) {
          console.log(
            `✅ Suggestions for "${partialQuery}": ${suggestions.slice(0, 3).join(', ')}`
          );
        } else {
