...
function analyzeTopOpenings() {
  ...
  openings.forEach((opening, index) => {
    ...
    const name =
      opening.name.length > 38 ? opening.name.substring(0, 35) + '...' : opening.name.padEnd(38);
    const moves =
      opening.moves.length > 40 ? opening.moves.substring(0, 37) + '...' : opening.moves;
  ...
  console.log(
    `• Total games in top 100: ${openings.reduce((sum, o) => sum + o.games_analyzed, 0).toLocaleString()}`
  );
  console.log(
    `• Average games per opening: ${Math.round(openings.reduce((sum, o) => sum + o.games_analyzed, 0) / 100).toLocaleString()}`
  );
  console.log(
    `• ECO root openings in top 100: ${rootOpenings.length}/100 (${Math.round(rootOpenings.length)}%)`
  );
  console.log(
    `• Non-root variations: ${100 - rootOpenings.length}/100 (${100 - rootOpenings.length}%)`
  );

  // Additional analysis: ECO root openings with popularity score of 10
  const allOpenings = Object.entries(popularityStats).map(([fen, stats]) => {
    const ecoInfo = findOpeningData(fen, ecoData);
    return {
      fen,
      popularity_score: stats.popularity_score,
  ...
  const rootOpeningsWithScore10 = allOpenings.filter(
    (o) => o.isEcoRoot && o.popularity_score === 10
  );
      ...
      .forEach((opening) => {
        ...
        const name =
          opening.name.length > 25
            ? opening.name.substring(0, 22) + '...'
            : opening.name.padEnd(25);
        const moves =
          opening.moves.length > 25 ? opening.moves.substring(0, 22) + '...' : opening.moves;
  ...
  const allRootOpeningsScore10 = allOpenings
    .filter((o) => o.isEcoRoot && o.popularity_score === 10)
    .sort((a, b) => b.games_analyzed - a.games_analyzed);
  ...
  const remainingRootOpeningsScore10 = allRootOpeningsScore10.filter((o) => !top100Fens.has(o.fen));
  ...
  console.log(
    `• Remaining ECO root openings (score 10, not in top 100): ${remainingRootOpeningsScore10.length}`
  );
  console.log(
    `• Total openings in comprehensive dataset: ${100 + remainingRootOpeningsScore10.length}`
  );
  console.log(`• Total ECO root openings with score 10: ${allRootOpeningsScore10.length}`);
  console.log(
    `• ECO root openings already in top 100: ${allRootOpeningsScore10.length - remainingRootOpeningsScore10.length}`
  );
