...
async function runIncrementalPipeline() {
    ...
    try {
        ...
        if (enrichedVideos.length > validEnrichedVideos.length) {
            console.log(`   Failed to enrich ${enrichedVideos.length - validEnrichedVideos.length} videos.`);
        ...
        const matchResults = await matcher.runMatchingWithVideos(validEnrichedVideos, { clearDb: false });
